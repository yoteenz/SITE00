...
function findFlowAPair(pipeline: MobileTwinPipelineState): {
  ...
  const blueprints = pipeline.blueprintTwins.filter((b) => b.structuralSource === 'FROZEN_COMPOSITION_STATE');
...
export function reconcileMobileTwinPipelineState(pipeline: MobileTwinPipelineState): MobileTwinPipelineState {
  ...
  if (pair || receipt) {
    ...
    const compositionResolved =
      composition ?? (compositionStateId ? next.compositionStates.find((c) => c.id === compositionStateId) : null);

    const needsUpgrade =
      !existing ||
      (existing.status !== 'FOUNDER_REVIEW_READY' && existing.status !== 'PARTIAL' && (pair !== null || receipt?.status === 'COMPLETE'));

    if (needsUpgrade && actualId && blueprintId && compositionResolved && next.designReference) {
