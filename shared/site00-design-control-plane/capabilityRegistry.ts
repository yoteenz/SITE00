...
import type { ManagedProjectRuntimeCapability, Site00ChangeExecutionClass } from './types.js';
...
export function classifyChangeExecution(
  ...
  projectKey: string,
): { executionClass: Site00ChangeExecutionClass; implementationMode: 'RUNTIME_BINDING' | 'SOURCE_REPO_CHANGE'; fallbackReason?: string } {
  if ((SOURCE_CODE_CHANGE_TYPES as readonly string[]).includes(changeType)) {
    return { executionClass: 'SOURCE_CODE_MATERIALIZATION', implementationMode: 'SOURCE_REPO_CHANGE' };
  }
  ...
  for (const op of operations) {
    ...
    if (needed === 'SOURCE_CODE_REQUIRED' || !caps.includes(needed)) {
      return {
        ...
        implementationMode: 'SOURCE_REPO_CHANGE',
  ...
  if ((RUNTIME_SAFE_CHANGE_TYPES as readonly string[]).includes(changeType)) {
    return { executionClass: 'RUNTIME_SAFE_BINDING', implementationMode: 'RUNTIME_BINDING' };
  }
