...
import { TerritorySpecimenCanvas } from '../../components/evolve/creative-direction/TerritorySpecimenCanvas';
...
export default function EvolveCreativeDirectionPage() {
  ...
  const reload = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await site00EvolveApi.creativeDirection(orgSlug);
  ...
  const submitDecision = async (type: 'APPROVE' | 'REFINE' | 'HYBRIDIZE' | 'REJECT') => {
    if (!payload) return;
    const territory = payload.engagement.territories[activeTerritory];
    setBusy(type);
    try {
      await site00EvolveApi.creativeDirectionDecision(orgSlug, {
        type,
      ...
      <div className="site00-cd">
        ...
        {payload && !loading ? (
          <>
            ...
            <footer className="site00-cd__footer">
