...
/** Approved desktop Origin — artboard shell, top nav, status rail, expanded panels. */
export function DesktopOrigin() {
  const { state, setHomeMode } = useSite00();
  const collapseExpandedPanel = useCallback(() => setHomeMode('origin'), [setHomeMode]);

  useOriginExpandedDismiss(state.homeMode, collapseExpandedPanel, true);

  return (
    <EnvironmentShell environmentId="ORIGIN_ENVIRONMENT">
      <div className="site00-origin-page site00-origin-page--desktop-artboard">
        <Site00AppShell
          locationLabel={SITE00_ORIGIN_COPY.locationLabel}
          showStatusStrip
          statusStrip={<StatusStrip layout="desktop" />}
        >
          <div
            ...
            style={{
              ...
              ['--site00-origin-expanded-panel-scale' as string]: String(
                SITE00_ORIGIN_DESKTOP_COMPOSITION.expandedPanelScale,
              ),
