...
import {
  bootstrapB49R4 as bootstrapB49R2,
...
describe('Expression Engine Sprint B4.9R2 — Single-artifact cinematic storyboard recovery', { timeout: 60000 }, () => {
  ...
  it('1. generation mode = REEL_FIRST_SINGLE_ARTIFACT (R4 canonical)', async () => {
  ...
  it('6. final storyboard asset count = 1', async () => {
    ...
    expect(result.reelArtifact?.rendered).toBe(true);
  });
  ...
  it('17. compiled reel-first storyboard prompt includes nine stills and key beats', async () => {
    const result = await bootstrapB49R2();
    const prompt = result.compiledPrompt;
    expect(prompt).toContain('NINE SELECTED STILLS');
    expect(prompt).toMatch(/snap[- ]back/i);
  ...
  it('30. founder review inactive in CI — store requires visual-authority-bound R4 candidate', async () => {
    const result = await bootstrapB49R2();
  ...
  it('35. storyboard 005 generated by current bootstrap (R4 — not founder-reviewable in CI)', async () => {
    const result = await bootstrapB49R2();
    expect(result.finalCinematicStoryboard?.storyboardId).toBe(ENTRY_002_FINAL_CINEMATIC_STORYBOARD_005_ID);
    expect(result.finalCinematicStoryboard?.generationMode).toBe('REEL_FIRST_SINGLE_ARTIFACT');
  ...
  it('35b. storyboard 003 preserved as historical FAILED_REEL_COHERENCE', async () => {
    await bootstrapB49R2();
    const historical003 = getStoryboard003HistoricalRecord();
    expect(historical003?.storyboardId).toBe(ENTRY_002_FINAL_CINEMATIC_STORYBOARD_003_ID);
    expect(historical003?.status).toBe('FAILED_REEL_COHERENCE');
  });

  it('36. telemetry notes pipeline test state in CI', async () => {
    const result = await bootstrapB49R2();
    expect(result.telemetryNote).toContain('PIPELINE_TEST_ONLY');
  });

  it('37. keyframes remain blocked without visual review ready even after LOVE_IT', async () => {
    await bootstrapB49R2();
