...
describe('P0.VR.TWINV3.0R8M mobile twin implementation pipeline', () => {
  ...
  it('7–11 compile consumes structured package; forbids raster authorities', async () => {
    ...
    expect(compiled.document.lineage).toBe(P0_VR_TWIN_V30R8M2R5_LINEAGE);
    expect(compiled.document.compilerGeneration).toBe(MOBILE_TWIN_IMPL_COMPILER_GENERATION_R8M2R5);
  ...
  it('24–26 implementation versioning + history in memory payload', async () => {
    ...
    expect(v1.build.implementationVersion).toBe(MOBILE_TWIN_IMPLEMENTATION_VERSION_FORENSIC_BLUEPRINT);
  ...
  it('28b twin preview local compile when implementation schema missing on server', async () => {
    ...
    expect(preview.document.compilerGeneration).toBe(MOBILE_TWIN_IMPL_COMPILER_GENERATION_R8M2R5);
