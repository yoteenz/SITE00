...
| `/projects/:slug/design/visual-system` | Isolated **staged** DESIGN workspace icon/symbol family review (P0.VR.DESIGN.GROK-VISUAL-SYSTEM1). Grok visual assets only — Opus owns live layout. Not wired into the live DESIGN shell. Boots without CTRL ROOM sign-in. Founder APPROVE before Composer implements. |
| `/projects/:slug/design/viewmode-icons` | Isolated **staged** Canonical / List icon-only pair (P0.VR.DESIGN.GROK-VIEWMODE-ICONS1). Does not replace the live text toggle until founder APPROVE. Boots without CTRL ROOM sign-in. |
