...
| `/projects/:slug/design/twin-opus-direct` | Isolated **Claude Opus 5** direct reconstruction of the founder golden (P0.VR.DESIGNBENCH.OPUS-DIRECT1). 768×1376 artboard scaled to fit; geometry measured off `public/site00/twin-v3-design-page-authority/founder-r5f2-ndxbook/mobile-master.jpg` (the golden master). Real DOM/CSS only — no raster cheat, no provider API. Boots without CTRL ROOM sign-in. **GROK-ASSET-OPUS1** swapped xerox/archival plates only; Opus shell, live text, icons, and geometry stay frozen. |
