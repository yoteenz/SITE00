...
| `/projects/:slug/design/twin-opus-direct` | Isolated **Claude Opus 5** direct reconstruction of the founder golden (P0.VR.DESIGNBENCH.OPUS-DIRECT1). 768×1376 artboard scaled to fit; geometry measured off `public/site00/twin-v3-design-page-authority/founder-r5f2-ndxbook/mobile-master.jpg` (the golden master). Real DOM/CSS only — no raster cheat, no provider API. Boots without CTRL ROOM sign-in. |
| `/projects/:slug/design/twin-sol-direct` | Isolated Sol **structural** reconstruction of the same golden (SOL-DIRECT1 / R1). **GROK-ASSET1** replaces placeholder plates; **GROK-ASSET1R1** redraws icons/symbols as reference-traced SVGs. Sol shell/section geometry stays frozen. Real DOM/CSS; no raster cheat; no CTRL ROOM guard. |
