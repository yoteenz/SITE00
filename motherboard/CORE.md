...
| `/projects/design` | DESIGN module hub (`DesignProjectsDesignHubPage`) |
| `/projects/design/:projectSlug` | **Founder-facing production DESIGN workspace** — `DesignProductionWorkspacePage` → `DesignWorkspaceCore` → `TwinOpusDirectScreen` (CURRENT/CONCEPT, PAGE SYSTEM REVIEW, PIPELINE/READINESS, CANONICAL/LIST, Opus/Grok docks). Nested sections: `/references|assets|pages|skins|history|more`. |
| `/projects/:slug/design` | Legacy path — redirects to `/projects/design/:slug` |
| `/projects/:slug/design/twin-opus-direct` | Twin founder-review surface — **same** `DesignWorkspaceCore` as production (not a separate visual fork); nested section routes mirror production |
| `/projects/:slug/design/reconstruction-lab` | Legacy reconstruction lab (`StudioWorldDesignWorkspace`) — **not** product DESIGN |
| `/projects/site00/design` | Redirect hub only (`Site00DesignHostRouteGate`) |
