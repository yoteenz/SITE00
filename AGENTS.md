...
**Mobile tunnel vs production:** Cloud preview hostname (Cloud Secrets) → Cloudflare tunnel → **localhost:5174**. Preview serves the **exact GitHub Actions `site00-production-dist` artifact** (same bytes as `deploy_frontend` on GoDaddy) via `.cursor/scripts/download-site00-ci-production-dist.sh` — **not** a one-off VM `npm run build` (wrong `app-build-id` / chunk hashes). Parity: `/release-manifest.json` on preview vs `https://site00.com/release-manifest.json` — **`bundleEntry` must match** (e.g. `index.CtFQ3tLv.js`). Modes: default **`ci`**; `SITE00_CLOUD_PREVIEW_MODE=local` (VM build with `GITHUB_SHA`); `dev` (HMR). Restart: `bash .cursor/scripts/restart-site00-preview-tunnel.sh` after env changes.
