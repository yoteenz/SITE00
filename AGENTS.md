...
Preview tunnel: `.cursor/environment.json` terminals `site00-vite` + `site00-preview-tunnel`. URL file: `/tmp/site00-cloud-preview-url.txt`. If the tunnel terminal dies or the preview host won’t load, run `bash .cursor/scripts/restart-site00-preview-tunnel.sh` (requires `SITE00_CLOUDFLARE_TUNNEL_TOKEN` in Cloud Secrets). Logs: `/tmp/site00-preview-tunnel.log`.

**Mobile tunnel vs production:** Cloud preview hostname (Cursor secret `SITE00_CLOUDFLARE_TUNNEL_HOSTNAME`) → Cloudflare tunnel → **localhost:5174**. The cloud environment now serves **`vite preview` of production `dist/`** (after `git fetch` + ff-only `main` + `npm run build`), **not** the old always-on Vite dev server — so Design should match the **same built bundle** as GoDaddy when both are on the same `main` commit. If GoDaddy has a **newer manual ZIP** than `main`, production leads the tunnel until you deploy again or restart the cloud preview terminal. Check parity: `/release-manifest.json` on the preview host vs `https://site00.com/release-manifest.json` (`commitSha`, `bundleEntry`). Agent HMR: set `SITE00_CLOUD_PREVIEW_MODE=dev` on the `site00-vite` terminal. iOS Safari app-switch often **fully reloads** the tab (in-memory UI lost; **localStorage** keeps Twin V2 / concepts). Twin V2 import drafts persist in **sessionStorage** (`site00:twin-v2-ui:v1:*`) when reopening Page Upgrade on the same page.
