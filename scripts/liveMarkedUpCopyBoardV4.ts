 ...
 * Uses background production job to avoid HTTP gateway timeout.
 */
...
async function pollJob(token: string, jobId: string) {
  ...
  while (Date.now() - started < MAX_WAIT_MS) {
    ...
    if (job.status === 'failed') {
      throw new Error(job.errorMessage ?? 'Production job failed');
    }

    await new Promise((r) => setTimeout(r, POLL_MS));
  }
  throw new Error('Timed out waiting for board v4 production job');
}
...
async function main() {
  ...
  console.log(`Start status: ${startBody.status ?? startBody.error ?? 'unknown'}`);

  let result = startBody;
  if (startRes.status === 202 && startBody.job?.id) {
    const job = await pollJob(token, startBody.job.id);
    result = job.result?.markedUpCopyBoardV4 ?? job.result ?? job;
  ...
  if (result.credentialExposed === true) {
    throw new Error('credentialExposed flag true');
  }

  if (result.status === 'BLOCKED_ON_PRODUCTION_SONNET_CREDENTIAL') {
    process.exit(2);
  }
