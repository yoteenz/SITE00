#!/usr/bin/env tsx
/**
 * Live THE MARKED-UP COPY brand-native visual pilot — ONE hero via Railway API.
 */
...
async function pollJob(token: string, jobId: string) {
  ...
  throw new Error('Timed out waiting for brand-native visual pilot job');
}

async function main() {
  console.log('=== BRAND-NATIVE VISUAL PILOT (ONE HERO) ===');
  ...
  const startRes = await adminFetch('/api/admin/site00-evolve', token, {
    method: 'POST',
    body: JSON.stringify({
      action: 'creative_direction_marked_up_copy_brand_native_visual_pilot',
      orgSlug: 'ndxbook',
      dryRun: false,
    }),
  });
  ...
  if (startRes.status === 202 && startBody.job?.id) {
    ...
    result = job.result?.markedUpCopyBrandNativeVisualPilot ?? job.result ?? job;
    result = { ...result, jobId: job.id, jobStatus: job.status };
  }

  const outPath = '/tmp/marked-up-copy-brand-native-pilot-result.json';
