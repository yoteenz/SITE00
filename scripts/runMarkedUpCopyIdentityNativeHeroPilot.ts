#!/usr/bin/env tsx
/**
 * Live THE MARKED-UP COPY identity-native hero pilot — ONE hero via Railway API.
 */
...
async function pollJob(token: string, jobId: string) {
  ...
  throw new Error('Timed out waiting for identity-native hero pilot job');
}

async function main() {
  console.log('=== IDENTITY-NATIVE HERO PILOT (ONE HERO) ===');
  ...
  const startRes = await adminFetch('/api/admin/site00-evolve', token, {
    method: 'POST',
    body: JSON.stringify({
      action: 'creative_direction_marked_up_copy_identity_native_hero_pilot',
      orgSlug: 'ndxbook',
      dryRun: false,
    }),
  });
  ...
  if (startRes.status === 202 && startBody.job?.id) {
    ...
    result = job.result?.markedUpCopyIdentityNativeHeroPilot ?? job.result ?? job;
    result = { ...result, jobId: job.id, jobStatus: job.status };
  }

  const outPath = '/tmp/marked-up-copy-identity-native-hero-pilot-result.json';
